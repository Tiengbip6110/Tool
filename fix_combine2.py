...
# Inject Letter text - properly escape using replace instead of re.sub repl string
letter_text = read_file('project/letter.txt')
letter_text_escaped = letter_text.replace('`', '\\`')

letter_js = f"""
function loadLetter() {{
    const text = `{letter_text_escaped}`;
    letterText = text.split(/\\n\\s*\\n/).map((p) => p.trim()).filter((p) => p !== "");
}}
"""
script_js = re.sub(r'async function loadLetter\(\) \{.*?\}', lambda _: letter_js, script_js, flags=re.DOTALL)
