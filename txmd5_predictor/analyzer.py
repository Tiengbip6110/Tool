class Analyzer:
    ...
    def predict(self, algo_name, data):
    ...
    def _simulate_and_optimize(self):
        ...
        for algo in self.algorithms:
            ...
            for i in range(5, len(self.history) - 1):
                historical_slice = self.history[:i+1]
                actual_next_result = self.history[i+1]['result']

                prediction = self.predict(algo, historical_slice)
    ...
    def process_new_session(self, session):
        ...
        if 'result' not in session:
             dice_sum = session.get('total', sum(session.get('dices', [])))
