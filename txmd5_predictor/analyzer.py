...
class Analyzer:
    def __init__(self):
        ...
        self.algorithms = {
            ...
            "markov_3": self._predict_markov_order_3,
            "dice_sum": self._predict_dice_sum,
            "dynamic_trend": self._predict_dynamic_trend,
            "dice_shadow": self._predict_dice_shadow,
            "streak_breaker": self._predict_streak_breaker,
            "rolling_variance": self._predict_rolling_variance,
            "pattern_match_4": self._predict_pattern_match_length_4,
    ...
    def _predict_markov_order_3(self) -> str:
        """Looks at what usually follows the last THREE results."""
        if len(self.history) < 4:
            return self._predict_markov_order_2()

        last_3 = (self.history[-3]["result"], self.history[-2]["result"], self.history[-1]["result"])
        counts = {"TAI": 0, "XIU": 0}

        for i in range(len(self.history) - 3):
            if (self.history[i]["result"], self.history[i+1]["result"], self.history[i+2]["result"]) == last_3:
                next_res = self.history[i+3]["result"]
                if next_res in counts:
                    counts[next_res] += 1
    ...
    def _predict_dice_shadow(self) -> str:
        """Uses 'bóng' (shadow) logic on the previous dice sum to predict next state."""
        if not self.history:
            return "TAI"

        # Shadow rules typically: 1->6, 2->7, 3->8, 4->9, 5->10, etc.
        # Here we do a simplified shadow offset based on sum mod 2 and other heuristics.
        last_point = self.history[-1].get("point", 11)
        # Bóng dương: +5
        shadow_point = (last_point + 5) % 18
        if shadow_point == 0: shadow_point = 18

        # Predict based on the shadow point
        if shadow_point > 10:
            return "TAI"
        else:
            return "XIU"

    def _predict_streak_breaker(self) -> str:
        """Predicts a break in a streak if the current streak length is >= 4."""
        if len(self.history) < 4:
            return self._predict_streak()
    ...
    def _predict_rolling_variance(self) -> str:
        """Calculates variance of last 5 points to determine stability/volatility."""
        if len(self.history) < 5:
            return self._predict_dynamic_trend()

        last_5_points = [s.get("point", 11) for s in self.history[-5:]]
        mean = sum(last_5_points) / 5.0
        variance = sum((x - mean) ** 2 for x in last_5_points) / 5.0

        last_point = last_5_points[-1]

        # If high variance (volatile), we expect a flip from the last state
        if variance > 10.0:
            return "TAI" if self.history[-1]["result"] == "XIU" else "XIU"
        else:
            # If low variance (stable), we expect continuation
            return self.history[-1]["result"]

    def _predict_pattern_match_length_4(self) -> str:
        """Looks for the exact sequence of the last 4 results in history."""
        if len(self.history) < 5:
            return self._predict_markov_order_3()

        last_4 = [h["result"] for h in self.history[-4:]]
        counts = {"TAI": 0, "XIU": 0}

        history_results = [h["result"] for h in self.history]
        for i in range(len(history_results) - 4):
            if history_results[i:i+4] == last_4:
                next_res = history_results[i+4]
                if next_res in counts:
                    counts[next_res] += 1
